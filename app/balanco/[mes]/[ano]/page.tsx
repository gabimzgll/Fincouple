  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/" className="text-rose-500 hover:text-rose-700">Voltar</Link>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl shadow p-5 border border-rose-100">
              <h2 className="text-lg font-semibold text-rose-600 mb-3">Gabi</h2>
              <div className="space-y-2 text-sm">
                ...
                {Object.entries(gabiBalanco.saidas_por_metodo).map(([m, v]) => (
                  <div key={m} className="flex justify-between pl-4 text-gray-400"><span>{m}</span><span>{formatCurrency(v)}</span></div>
                ))}
            ...
            <div className="bg-white rounded-2xl shadow p-5 border border-blue-100">
              <h2 className="text-lg font-semibold text-blue-600 mb-3">Rafa</h2>
              <div className="space-y-2 text-sm">
                ...
                {Object.entries(rafaBalanco.saidas_por_metodo).map(([m, v]) => (
                  <div key={m} className="flex justify-between pl-4 text-gray-400"><span>{m}</span><span>{formatCurrency(v)}</span></div>
                ))}
                <div className="border-t pt-2 flex justify-between font-bold">
                  <span>Sobra</span>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t pt-2">
                {saldoCasal.quem_deve ? (
                  <p className="font-bold text-amber-700 text-center">{saldoCasal.quem_deve} deve {formatCurrency(Math.abs(saldoCasal.saldo_final))} para {saldoCasal.quem_recebe}</p>
                ) : (
                  <p className="font-bold text-green-700 text-center">Quites!</p>
                )}
              </div>
            </div>
          </div>
                ...
                {effective.map(({ transaction: t, valor }) => (
                  <li key={`${t.id}-${valor}`} className="py-3 flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      {t.tipo === 'parcelado' && <p className="text-xs text-rose-400">Parcelado em {t.parcelas}x</p>}
                    </div>
